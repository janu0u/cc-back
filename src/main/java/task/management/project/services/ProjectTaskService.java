...
import task.management.project.models.ProjectTask;
import task.management.project.repositories.ProjectTaskRepository;
...
public class ProjectTaskService {
  ...
  public ProjectTask addProjectTask(String projectIdentifier, ProjectTask projectTask,
    ...
    Backlog backlog = projectService.findProjectByIdentifier(projectIdentifier, username)
        .getBacklog(); //backlogRepository.findByProjectIdentifier(projectIdentifier);
    //set the bl to pt
...
// TODO change how project sequence is handled
...
//    projectTask.setProjectSequence(backlog.getProjectIdentifier() + "-" + BacklogSequence);
    ...
    if (projectTask.getStatus() == "" || projectTask.getStatus() == null) {
      ...
      // TODO create enum for project status
      projectTask.setStatus("TO_DO");

    }

    //Fix bug with priority in Spring Boot Server, needs to check null first
    if (projectTask.getPriority() == null || projectTask.getPriority()
        == 0) { //In the future we need projectTask.getPriority()== 0 to handle the form
